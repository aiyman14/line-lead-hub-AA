...
export default function SewingUpdate() {
  ...
  // Auto-fill target when work order is selected
  useEffect(() => {
    if (selectedWorkOrder) {
      const wo = workOrders.find(w => w.id === selectedWorkOrder);
      if (wo?.target_per_day) {
        setTargetQty(wo.target_per_day.toString());
      }
    }
  ...
  async function fetchFormData() {
    ...
    try {
      const [linesRes, workOrdersRes, stagesRes, blockerTypesRes] = await Promise.all([
        ...
        supabase
          .from('work_orders')
          .select('id, po_number, buyer, style, item, order_qty, target_per_hour, target_per_day, line_id')
          .eq('factory_id', profile.factory_id)
          .eq('is_active', true)
          .order('po_number'),
        supabase
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!outputQty || parseInt(outputQty) < 0) {
      toast({ variant: "destructive", title: "Please enter a valid output quantity" });
      return;
    }
    ...
    try {
      const insertData: any = {
        ...
        shift,
        output_qty: parseInt(outputQty) || 0,
        ...
        reject_qty: parseInt(rejectQty) || 0,
        rework_qty: parseInt(reworkQty) || 0,
      ...
      toast({
        ...
        description: "Your sewing production update has been recorded.",
      });
    ...
    <div className="p-4 lg:p-6 max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold">Sewing Line Update</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Stage & Quality</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="mobile-form-field">

...
CREATE INDEX idx_app_error_logs_factory_id ON public.app_error_logs (factory_id);

-- Enable RLS
ALTER TABLE public.app_error_logs ENABLE ROW LEVEL SECURITY;

-- INSERT: permissive — errors must log even during partial auth
CREATE POLICY "Anyone can insert error logs"
  ON public.app_error_logs
  FOR INSERT
  WITH CHECK (true);

-- SELECT: only admin/owner can read error logs for their factory
...
-- UPDATE: only admin/owner can acknowledge errors
...
-- DELETE: only admin/owner can delete error logs
